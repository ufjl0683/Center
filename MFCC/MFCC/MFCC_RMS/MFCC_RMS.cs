    ...
    class MFCC_RMS:Comm.MFCC.MFCC_Base
    {
        ...
        void MFCC_RMS_OnTCReport(object tc, Comm.TextPackage txt)
        {
            ...
            else if (txt.Text[0] == 0xaa)   //儀控啟動結束
            {
                ...
                dev.isDisplayOff=(txt.Text[5] ==2)?true:false;
                ConsoleServer.WriteLine(dev.DeviceName + "," + (txt.Text[5] == 2 ? "儀控中止" : "儀控啟動"));
                if (txt.Text[5] == 2)
                    r_host_comm.setRMS_RampControl_Stop(dev.DeviceName);
                else
                    r_host_comm.setRMS_RampControl_Start(dev.DeviceName);
            }
            else if (txt.Text[0] == 0xAF && txt.Text[1] == 0x02)
            {
                ...
                if (l_mode == 1)
                    ...
                    r_host_comm.setRMS_LTR_Start(dev.DeviceName);


                else

                    r_host_comm.setRMS_LTR_Stop(dev.DeviceName);
