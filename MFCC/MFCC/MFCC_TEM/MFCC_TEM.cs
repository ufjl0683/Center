 	  ...
 	  try
                ...
                switch (txt.Text[0])
                {
                    ...
                    case 0x10:  //filealarm
                     ...
                     //   key = r["tunnel"].ToString() + "-" + r["place"] + "-" + r["div"];
                        ...
                        if (!hsFireAlarmData.Contains(fdata.Key))
                        {
                            ...
                            if (fdata.status != 0)
                                sendHostEvent(tc.DeviceName,fdata);
                            //  this.r_host_comm.setTemEvent(tc.DeviceName, data);

                        }
                        else
                        {
                            if((hsFireAlarmData[fdata.Key] as FireAlarmData).setStatus(Convert.ToInt32(r["status"])))
                    ...
                    case 0x13:   //power
                       // key = r["tunnel"].ToString() + "-" + r["place"];
                        ...
                        if (!hsPowerAlarmData.Contains(pdata.Key))
                        {
                            ...
                            if (pdata.status != 0)
                                sendHostEvent(tc.DeviceName, pdata);
                            //   this.r_host_comm.setTemEvent(tc.DeviceName, data);

                        }
                        else
                        {
                            if((hsPowerAlarmData[pdata.Key] as PowerAlarmData).setStatus(Convert.ToInt32(r["status"])))
                    ...
                    case 0x14:  //security
                       // key = r["tunnel"].ToString() + "-" + r["place"]+"-"+r["card_reader"];\
                        ...
                        if (!hsSecurityAlarmData.Contains(sdata.Key))
                        {
                            ...
                            if (sdata.status != 0)
                                sendHostEvent(tc.DeviceName, sdata);
                            // this.r_host_comm.setTemEvent(tc.DeviceName, data);

                        }
                        else
                        {
                           if( (hsSecurityAlarmData[sdata.Key] as SecurityAlarmData).setStatus(Convert.ToInt32(r["status"])))
                    ...
                    case 0x15:
                        ...
                        if (!hsMonitorAlarmData.Contains(mdata.Key))
                        {
                            ...
                            if (mdata.status != 0)
                                sendHostEvent(tc.DeviceName, mdata);
                        ...
                        {
                            if((hsMonitorAlarmData[mdata.Key] as MonitorAlarmData).setStatus(Convert.ToInt32(r["status"])))
                    ...
                    case 0x12:  //light
                       // key = r["tunnel"].ToString() + "-" + r["place"] + "-" + r["div"];
                        ...
                        if (!hsLightAlarmData.Contains(ldata.Key))
                        {
                            ...
                            //  this.r_host_comm.setTemEvent(tc.DeviceName, data);
                            if (ldata.damaged!= 0)
                                sendHostEvent(tc.DeviceName, ldata);
                        }
                        else
                        {
                           if( (hsLightAlarmData[ldata.Key] as LightAlarmData).setStatus(Convert.ToInt32(r["required"]), Convert.ToInt32(r["damaged"])))
                    ...
                    case 0x11:  //air 
                        ...
                        if (!hsAirAlarmData.Contains(vidata.Key))
                        {
                            hsAirAlarmData.Add(vidata.Key, vidata);
                            vidata.OnEvent += new OnEventHandler(data_OnEvent);
                            vidata.OnEventStop += new OnEventHandler(data_OnEventStop);
                            ConsoleServer.WriteLine("新增空氣品質分區:" + vidata);
                           
                        }
                        else
                        {
                            if ((hsAirAlarmData[vidata.Key] as AirAlarmData).setData(vidata.density, System.Convert.ToInt32(r["vi_level_2"])))
                                ConsoleServer.WriteLine("空氣品質:" + vidata);
                        }


                        if (!hsAirAlarmData.Contains(codata.Key))
                        {
                            hsAirAlarmData.Add(codata.Key, codata);
                            codata.OnEvent += new OnEventHandler(data_OnEvent);
                            codata.OnEventStop += new OnEventHandler(data_OnEventStop);
                            ConsoleServer.WriteLine("新增空氣品質分區:" + codata);

                        }
                        else
                        {
                            if ((hsAirAlarmData[codata.Key] as AirAlarmData).setData(codata.density, System.Convert.ToInt32(r["co_level_1"])))
                                ConsoleServer.WriteLine("空氣品質:" + codata);
                        }


                        if (!hsAirAlarmData.Contains(nodata.Key))
                        {
                            hsAirAlarmData.Add(nodata.Key, nodata);
                            nodata.OnEvent += new OnEventHandler(data_OnEvent);
                            nodata.OnEventStop += new OnEventHandler(data_OnEventStop);
                            ConsoleServer.WriteLine("新增空氣品質分區:" + nodata);

                        }
                        else
                        {
                            if ((hsAirAlarmData[nodata.Key] as AirAlarmData).setData(nodata.density, System.Convert.ToInt32(r["no_level_3"])))
                                ConsoleServer.WriteLine("空氣品質:" + nodata);
                        }

                        if (!hsAirAlarmData.Contains(no2data.Key))
                        {
                            hsAirAlarmData.Add(no2data.Key, no2data);
                            no2data.OnEvent += new OnEventHandler(data_OnEvent);
                            no2data.OnEventStop += new OnEventHandler(data_OnEventStop);
                            ConsoleServer.WriteLine("新增空氣品質分區:" + no2data);

                        }
                        else
                        {
                            if ((hsAirAlarmData[no2data.Key] as AirAlarmData).setData(no2data.density, System.Convert.ToInt32(r["no2_level_4"])))
                                ConsoleServer.WriteLine("空氣品質:" + no2data);
                        }

                        if (!hsAirAlarmData.Contains(noxdata.Key))
                        {
                            hsAirAlarmData.Add(noxdata.Key, noxdata);
                            noxdata.OnEvent += new OnEventHandler(data_OnEvent);
                            noxdata.OnEventStop += new OnEventHandler(data_OnEventStop);
                            ConsoleServer.WriteLine("新增空氣品質分區:" + noxdata);

                        }
                        else
                        {
                            if ((hsAirAlarmData[noxdata.Key] as AirAlarmData).setData(noxdata.density, System.Convert.ToInt32(r["nox_level_5"])))
                                ConsoleServer.WriteLine("空氣品質:" + noxdata);
                        }
                        ...
                        //        if (a.level != 0)
                        //            sendHostEvent(tc.DeviceName, a);
                        //    }
                        //    else
                        //    {
                        //       if( (hsAirAlarmData[a.Key] as AirAlarmData).setData(a.density, a.level))
        ...
        void data_OnEventStop(object sender)
        {
            try
            {
                if (sender is FireAlarmData)
                    ...
                    ConsoleServer.WriteLine("監控分區結束" + sender);
        ...
        void data_OnEvent(object sender)
        {
            try
            {
                ...
                else if(sender is MonitorAlarmData)
                    ConsoleServer.WriteLine("監控分區" + sender);
