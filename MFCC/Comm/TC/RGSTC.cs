    ...
    public class RGSTC :OutputTCBase
    {
      ...
      //  public event OnOutputChangedHandler OnOutputChanged;
            ...
            : base(protocol, devicename, ip, port, deviceid, hw_status,opmode,opstatus,comm_state)
       {
        ...
        void RGSTC_OnTCReceiveText(object tc, TextPackage txt)
        {

            if (txt.Text[0] == 0x56)
                RGS_LedTest_Report(txt.Text, this.DeviceName);
            //throw new Exception("The method or operation is not implemented.");
        }

         

        private void RGS_LedTest_Report(byte[] txt, string devname)//56
        {
            int ErrorSum = 0;
            string SqlLedTest = "";
            ...
            if (ErrorSum != 0)
            {
                SqlLedTest = "INSERT INTO db2inst1.TBLLEDTest_RGS(DEVICENAME,TIMESTAMP,BADSETNO) VALUES('" + devname + "','" + RemoteInterface.DbCmdServer.getTimeStampString(t) + "'," + ErrorSum + ") ";
                this.InvokeDBDemand(SqlLedTest);
                for (int i = 7; i < txt.Length; i = i + 2)
                {
                    SqlLedTestDetail = "INSERT INTO db2inst1.tblLEDTestDetail_RGS(DEVICENAME,TIMESTAMP,X,Y) VALUES('" + devname + "','" + RemoteInterface.DbCmdServer.getTimeStampString(t) + "'," + txt[i] + "," + txt[i + 1] + ")";
            ...
            try
                {
                    ...
                    lock (currDispLockObj)
                    {
                            ...
                            {
                                this.InvokeOutPutWrongEvent(curr_display_data.ToString(), curr_display_data.ToString());
                            }
