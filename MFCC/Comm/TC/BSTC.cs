  ...
  public   class BSTC : TCBase
    {
      ...
      int curr_slope = -1, curr_shift = -1,curr_degree=0,curr_sink=-1;
        ...
        DataSet currData;
        public delegate void BSCycleDataHandler(Comm.TC.BSTC tc, System.DateTime dt,int response_type,int slope,int shift,int sink,int degree);
              ...
              try
              {
                  if (txt.Text[0] == 0x68  )   // cycle data  and 主動回報
                  {
                     ...
                     // this.currData = ds0x30;
                      ...
                      int respones_type = System.Convert.ToInt32(ds0x68.Tables[0].Rows[0]["bs_degree"]);

                      if (On_BS_CycleData != null)
                          this.On_BS_CycleData((BSTC)tc, dt, respones_type, curr_slope, curr_shift, curr_sink, curr_degree);
                  }
                     

                  else if (txt.Text[0] == 0x6a)  //event
                  {
                  //    System.DateTime dt = new DateTime(System.DateTime.Now.Year, txt.Text[6], txt.Text[7], txt.Text[8], txt.Text[9], 0);
                      DataSet ds0x6a = m_protocol.GetSendDsByTextPackage(txt, CmdType.CmdReport);
                     ...
                     // curr_day_var = System.Convert.ToInt32(ds0x32.Tables[0].Rows[0]["day_var"]);
                      ...
                      if (this.On_DegreeChange != null)
                          this.On_DegreeChange(this, DateTime.Now, ds0x6a);
