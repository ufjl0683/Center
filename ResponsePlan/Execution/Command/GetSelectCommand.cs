...
namespace Execution.Command
{
    internal class GetSelectCmd
    {
        ...
        public static ICommand getCMSCategory(int ruleid,int alarmclass,string devtype,int distance,string devName,string MegType,string lineid,string location,string devLine)
        {
            ...
            if (devLine != lineid || location == "S") //非主線讀取支線
            {
        ...
        public static ICommand getRGSCategory(int ruleid, int alarmclass, string devtype, int distance, string devName, string MegType, string lineid,string location,string devLine)
        {
            ...
            if (devLine != lineid)//平面道路或非主線讀取支線
            {
        ...
        public static ICommand getEndSectionName(string lineid,string direction,int mile)
        {
            ...
            com.TblNames += string.Format(" {0}.{1} s  ", schema, DB2TableName.vwGroupSection);
            com.TblNames += string.Format(" left join(select Divisionid,Divisionname,mileage from {0}.{1}) g1 on s.start_divisionid=g1.divisionid  ", schema, DB2TableName.vwGroupDivision);
            com.TblNames += string.Format(" left join(select Divisionid,Divisionname,mileage from {0}.{1}) g2 on s.end_divisionid=g2.divisionid ", schema, DB2TableName.vwGroupDivision);
