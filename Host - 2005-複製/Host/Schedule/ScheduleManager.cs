   ...
   public  class ScheduleManager
    {
       ...
       {
           OdbcConnection cn = new OdbcConnection(Global.Db2ConnectionString);
           OdbcCommand cmd = new OdbcCommand("select schid,schname,schtype,isduration,duration,week1,week2,week3,week4,week5,week6,week7,starttime from tblSchConfig  where schid="+sschid);
           OdbcDataReader rd;
           cmd.Connection = cn;
               ...
               //  int inx = 0;
                 while (rd.Read())
                 {
                     ...
                     if (!rd.IsDBNull(Global.getFiledInx(rd, "command")))
                         ...
                         command = "";
                     if(command=="")
                      ary.Add(  new ScheduleJob(devName, subschid, null));
                     else
                      ary.Add(new ScheduleJob(devName,subschid,RemoteInterface.Utils.Util.StringToObj(command)));
