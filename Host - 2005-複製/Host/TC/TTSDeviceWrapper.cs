           ...
           : base(mfccid, devicename, deviceType, ip, port, location, lineid, mile_m, hw_status, opmode, opstatus, direction)
       {
            ...
            for (int i = 0; i < travelDisplaySettingData.Length; i++) //calc  travel time
            {
                ...
                for (int j = 0; j < travelDisplaySettingData[i].detailData.Length; j++)
                {
                    ...
                    if (sec < 0)   // exit when one of travel time invalid
                    {
                     ...
                     int tmpsec=Program.matrix.line_mgr[ddata.lineid].getAVI_TravelTime(ddata.dir,ddata.startMile,ddata.endMile);
                     if (tmpsec < 0)
                         avitime[i] = -1;
                     else if (avitime[i] != -1)
                         avitime[i] += tmpsec;
        ...
        public void loadTravelSetting()
        {
            ...
            try
            {
                ...
                cn.Open();
                cmdMain.CommandText = "select DISPLAY_PART ,g_code_id,message1,message2,msg1forecolor,msg2forecolor,msg1backcolor,msg2backcolor,uppertravelTime,lowerTravelTime,offset,enable from tblRGSTravelTime where devicename='" + this.deviceName + "' order by display_part";
