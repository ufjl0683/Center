...
namespace Execution.Command
{
    internal class GetInsertCmd
    {
        ...
        private static InsertCommand setTbl_FiledName(InsertCommand insertCmd)
        {
            ...
            str += " originaleventid,MC_ID,Inc_notify,BlockTypeid,MC_Memo ";
        ...
        /// <param name="dr"></param>
        /// <returns></returns>
        public static InsertCommand setIIPEventCmd(System.Data.Odbc.OdbcDataReader dr)
        {
            ...
            str += string.Format("{0},timestamp('{1}'),timestamp('{2}'),{3},'{4}',", mode, DateTime.Parse(dr[4].ToString()).ToString("yyyy-MM-dd HH:mm:ss"), DateTime.Parse(dr[4].ToString()).ToString("yyyy-MM-dd HH:mm:ss"), notify, dr[8].ToString());//登入模式,發生時間,通報時間,通報來源,通報設備
            ...
            str += string.Format("'{0}'", dr[26].ToString());//施工訊息 
        ...
        public static InsertCommand setMoveToIIPEvnetCmd(string notifier, DateTime timeStamp, string lineID, string directionID, int startMileage, int endMileage,string blockTypeId,string blocklane,int eventID)
        {
            ...
            insertCmd.FiledNames += "OriginalEventID,BlockTypeId ";
            ...
            insertCmd.WhereCon += string.Format(" {0},'{1}' ", eventID, blockTypeId);
            return insertCmd;
        } 
