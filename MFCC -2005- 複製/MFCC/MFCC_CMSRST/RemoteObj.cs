...
{
   public  class RemoteObj:Comm.MFCC.RemoteMFCCBase,RemoteInterface.MFCC.I_MFCC_CMSRST
    {

       public override Comm.MFCC.MFCC_Base getMFCC_base()
       {
          // throw new Exception("The method or operation is not implemented.");
           return Program.mfcc_cmsrst;
       }
       ...
       public void SendDisplay(int inx,string devName, int g_code_id, int hor_space, string mesg, byte[] colors)
       {
           try
           {
               int ver_no = 1;
               ...
               this.SendDisplay(inx,devName, g_code_id, hor_space, mesg, colors, vspaces);
       ...
       public void SendDisplay(int inx,string devName, int g_code_id, int hor_space, string mesg, byte[] colors, byte[] v_spaces)
       {
           //throw new Exception("The method or operation is not implemented.");
           try
           {
               // ConsoleServer.WriteLine("from host1");
               Comm.TC.CMSRSTTC tc = (Comm.TC.CMSRSTTC)Program.mfcc_cmsrst.getTcManager()[devName];
               checkAllowConnect(tc);
               tc.TC_SendDisplay(inx, g_code_id, hor_space, mesg, colors, v_spaces);
               ConsoleServer.WriteLine(string.Format(devName + " g_code_id:{0} hos_space:{1} mesg:{2}",  g_code_id, hor_space, mesg));
           }
           catch (Exception ex)
           {
               ConsoleServer.WriteLine(devName + "," + ex.Message);
       ...
       // try
       //    {
       //        Comm.TC.CMSTC tc =(Comm.TC.CMSTC)Program.mfcc_cmsrst.getTcManager()[devName];
       //        checkAllowConnect(tc);
       ...
       public void GetCurrentTCDisplay(string devName, int inx,ref int dataType, ref int g_code_id, ref int hor_space, ref string mesg, ref byte[] colors, ref byte[] vspaces)
       {
           try
           {
               Comm.TC.CMSRSTTC tc = (Comm.TC.CMSRSTTC)Program.mfcc_cmsrst.getTcManager()[devName];
               ...
               System.Data.DataSet ds = tc.TC_GetDisplay(inx);

               //if (ds.Tables[0].Rows[0]["func_name"].ToString() == "get_display_control")
               //{
                  // icon_id = System.Convert.ToInt32(ds.Tables[0].Rows[0]["icon_code_id"]);
                   dataType = System.Convert.ToInt32(ds.Tables[0].Rows[0]["data_type"]);
                   inx = System.Convert.ToInt32(ds.Tables[0].Rows[0]["idx"]);
               //}
               //else if (ds.Tables[0].Rows[0]["func_name"].ToString() == "get_CMS_display")
               //{
               //    icon_id = 0;
