...
namespace RemoteInterface.HWStatus
{
    ...
    public enum CSLS_HW_Status_Bit_Enum
    {
        DeviceErr = 0,
        CabineteOpen = 1,
        PortableTest = 2,
   ...
   public  class CSLS_HW_StatusDesc:I_HW_Status_Desc
    {
       ...
       byte[] m_status;
            ...
            public CSLS_HW_StatusDesc(string devName, byte[] status, byte[] diff)
            {
       ...
       public CSLS_HW_StatusDesc(string devName, byte[] status)
         ...
         public string getDesc(int bitinx)
            {
                ...
                return ((CSLS_HW_Status_Bit_Enum)bitinx).ToString();
            }
            ...
            public System.Collections.IEnumerable getEnum(byte[] indexs)  //取得所有錯誤訊息的位元咧舉值
            {
                ...
                {
                    if (aryInx.Get(i))
                        yield return (CSLS_HW_Status_Bit_Enum)i;
                }
            }
