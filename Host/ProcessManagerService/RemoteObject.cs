  ...
  public   class RemoteObject:RemoteInterface.RemoteClassBase,RemoteInterface.I_ProcessManager
    {
       ...
       public DataTable getProcessStatus()
        {
            ...
            //{
                try
                {
                    ProcessManagerService.Ds ds = new Ds();
                    foreach ( ProcessWrapper p  in Service1.hash_process.Values )
                    {
      ...
      public void SetProcessRunningState(string pName,int state)
      {
          try
          {
              ProcessWrapper p = Service1.hash_process[pName] as ProcessWrapper;
      ...
      public bool IsAllProcessOk()
      {
          lock (Service1.ds)
          {
              try
              {
                  ...
                  foreach (Ds.tblProcessInfoRow r in Service1.ds.tblProcessInfo.Rows)
                  {
                      ProcessWrapper pw=(ProcessWrapper)Service1.hash_process[r.process_name];
