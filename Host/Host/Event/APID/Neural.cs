  ...
  public  class Neural
    {
      ...
      System.Collections.Generic.List<TrainData> traindatas = new List<TrainData>();
        public Neural()
        {
            new System.Threading.Thread(TrainTask).Start();
        }
       ...
       public int GetApidEvent(double[] input)
        {
            if (!IsTrained)
                return 0;
           return  network.Compute(input)[0] > 0.95?1:0;
        }
        public  void ExactTrainingData()
        {
            TrainData[] tdatas;
            ...
            for (int i = 1; i < tdatas.Length; i++)
            {
                ...
                input[i - 1][1] = spddiff;
                input[i - 1][2] = occdiff;
                input[i - 1][3] = u_voldifft;
                input[i - 1][4] = u_spddifft;
                input[i - 1][5] = u_occdifft;
                input[i - 1][6] = d_voldifft;
                input[i - 1][7] = d_spddifft;
