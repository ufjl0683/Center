   ...
   public  class JamManagerOld
    {
       ...
       public JamManagerOld(DevcieManager devMgr)
       {
...
//  檢查所有壅塞程度為 3 的車輛偵測器
           ...
           //加入新的壅塞範圍
            foreach (JamRange r in tmp)
            {
                ...
                //add event mgr  here
              //  Util.SysLog("sys.log",DateTime.Now+":"+ r.ToString());
                Program.matrix.event_mgr.AddEvent(r);
               // Util.SysLog("sys.log", DateTime.Now + ":" + r.ToString()+ "add completed!" );
            }
