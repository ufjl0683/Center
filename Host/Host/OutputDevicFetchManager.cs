   ...
   public class OutputDevicFetchManager
    {
       ...
       public FetchDeviceData[] Fetch(string[] deviceTypes, string lineId, string direction,int mileage, int segCnt, int sysSegCnt, bool IsBranch)
       {
           ...
           int[] SysSegCntLog = new int[sysSegCnt];
             ...
             int seg_cnt=0, sys_seg_cnt=0;
               ...
               do
               {
                   if (p.getDevType() == "C")
                   {
                       ...
                       if (seg_cnt > 2)  // 太近不算
                       {
                           SysSegCntLog[sys_seg_cnt++] = seg_cnt;
               ...
               if(sysSegCnt>0)  //計算係包含系統交流道至少需要幾個 seg
               {
                   for (int i = 0; i < SysSegCntLog.Length; i++)
                       if (SysSegCntLog[i] > segCnt)
                           segCnt = SysSegCntLog[i];
           ...
           {
               ArrayList tmplist;
              tmplist= Search(type,FindFirst_Position(lineId,direction,mileage),null,segCnt,0,IsBranch,0);
           ...
           foreach (string devType in deviceTypes)
           {
            ...
            for (int i = 0; i < retAry.Count; i++)
            {
                ...
                ret[i] = new FetchDeviceData(wraper.deviceName, 0,wraper.lineid,wraper.direction,wraper.mile_m,
                    ...
                    Program.matrix.line_mgr[lineId].getSectionByMile(wraper.direction,wraper.mile_m).minSpeed,wraper.deviceType);
            }
       ...
       private void loadMainLineOutDevice(TC.DevcieManager devMgr)
       {
           ...
           foreach (DeviceBaseWrapper dev in devMgr.getAllDeviceEnum())
           {

               if (dev is CCTVDeviceWrapper)
               {
           ...
           System.Data.Odbc.OdbcCommand cmd = new System.Data.Odbc.OdbcCommand("Select divisiontype,lineid1,direction1,mileage1,lineid2,direction2,mileage2 from tblcloverleaf");
           cmd.Connection = cn;
           try
           {
               cn.Open();
