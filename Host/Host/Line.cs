...
namespace Host
{
    public class Line
    {
        ...
        System.Collections.ArrayList aryAllSections = System.Collections.ArrayList.Synchronized(new System.Collections.ArrayList());
        ...
        public int getTotalSecCnt()
        {
            return aryAllSections.Count;
        }
        public /*Section[]*/ System.Collections.IEnumerable getAllSectionEnum()
        {
            ...
            //Section[] secs = new Section[aryAllSections.Count];
            //for (int i = 0; i < aryAllSections.Count; i++)

            //    secs[i] = aryAllSections[i] as Section;
            //return secs;
        ...
        public void getAllTrafficData(string dir,int startMile,int endMile,ref int volume, ref int speed, ref int occpuancy, ref int jamlevel, ref int traveltime,ref int lowerTravelTime,ref int upperTravelTime)
        {
            ...
            {
                ((Host.UnitRoad)this.hsUnitRoads[this.direction.IndexOf(dir)][i]).getAllVDTrafficData(ref refVDLst,ref vol,ref spd, ref occ, ref jmlvl/*, ref ttime*/,startMile,endMile);
         ...
         //    return;
            ...
            else
            {
                ...
                //else
                //{
                //    speed = occpuancy = jamlevel = volume = -1;
                //}
            ...
            if (traveltime != -1)
            {
                speed = this.getSpaceSpeed(dir, startMile, endMile); //2013/2/25
               //  2013/2/25    speed = System.Convert.ToInt32(((double)System.Math.Abs(endMile - startMile) / traveltime * 3.6));
        ...
        public int getSpaceSpeed(string dir, int startmile_m, int endmile_m)
        {
         ...
         //   int totalvol = 0;
            ...
            for (int i = startmile_m / 1000; i <= endmile_m / 1000; i++)
            {
                ...
                unit.getVDSpaceData(ref unitvol, ref unitsec,startmile_m,endmile_m);
            ...
            for (int i = startmile_m / 1000; i <= endmile_m / 1000; i++)
            {
                ...
                if(unit==null)
                    throw new Exception(this.lineid + "," + dir + i + "公里處無對應單位路段");
                     ...
                     unitsec = unit.getVD_TravelTime(startmile_m,endmile_m);
                if (unitsec >= 0)
                {
                    if (i == startmile_m / 1000)
