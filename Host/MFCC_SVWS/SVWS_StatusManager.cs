   ...
   public class SVWS_Status
    {
     ...
     public int no;
       ...
       public void setPDStatus(int place, int group, int kind, int dir, int run_status, int hw_status, int no)
       {
           this.type = "P";
           this.place = place;
           this.group = group;
        ...
        public string GetInsertStatement()
        {
            ...
            if (type == "A")
            {
                sql = "insert into tblsvwsconfig (type,place,group,kind,dir,density,level,no) values('{0}',{1},{2},{3},{4},{5},{6},{7})";
                return string.Format(sql, type, place, group, kind, dir, density, level,no);

            }
            else if (type == "D")
            {
                sql = "insert into tblsvwsconfig (type,place,group,kind,dir,run_status,hw_status,no) values('{0}',{1},{2},{3},{4},{5},{6},{7})";
                return string.Format(sql, type, place, group, kind, dir, run_status, hw_status,no);
        ...
        public string GetUpdateStatement()
        {
             ...
             {
                 sql = "update tblsvwsconfig set run_status={0},hw_status={1} where type='P' and place={2} and group={3} and kind={4} and dir={5} and no={6}";
                 return string.Format(sql, run_status, hw_status, place, group, kind, dir,no);
             }
           ...
           public string GetInsertLogSql()
           {
              ...
              //  return;
               ...
               else if (type == "P")
               {
                   ...
                   sql = "insert into tblsvwslog (type,place,group,kind,dir,run_status,hw_status,timestamp,no) values('{0}',{1},{2},{3},{4},{5},{6},'{7}',{8})";
                   return string.Format(sql, type, place, group, kind, dir, run_status, hw_status, DbCmdServer.getTimeStampString(DateTime.Now),no);
               }
        ...
        public SVWS_Status(System.Data.DataSet ds)
        {
            ...
            else if (r["func_name"].ToString() == "PD_signal_status_reports")
            {
                type = "P";
                run_status = System.Convert.ToInt32(r["run_status"]);
                hw_status = System.Convert.ToInt32(r["hw_status"]);
                no = System.Convert.ToInt32(r["no"]);
            }
            else
        ...
        public string Key
        {
            ...
            get
            {
                return type + "-" + place.ToString() + "-" + group.ToString() + "-" + kind.ToString() + "-" + dir.ToString()+"-"+no;
            }
        }
