   ...
   public class VDTC:TCBase
    {
      ...
      // private Queue QueOneMinVDData = new Queue(10);
       ...
       private void getLast5VD_Data() //取得最近五分鐘資料
       {
           ...
           for (int i = 0; i < 5; i++)
           {
               ...
               try
               {
                  SendPackage pkg;
                   ...
                   {
                       Console.WriteLine(pkg.result.ToString() + pkg.ToString());
                   }
           ...
           Console.WriteLine("five min avg:" + fiveMinData.ToString());
       ...
       private  void sendCycleSettingData() //傳送傳輸週期設定
       {
           ...
           {
               Console.WriteLine("In VD 1min task:"+ex.Message);
           }
       }
